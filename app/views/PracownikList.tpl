...
<body class="home">
    ...
    <section class="box special ">
        ...
        {if !empty($employees)}    
            <form id="search-form" class="pure-form pure-form-stacked" onsubmit="ajaxPostForm('search-form', '{url action="searchPracownik"}', 'table');
                          return false;">
                <ul class="actions special">
                    <li> Znajdź pracownika:
                        <input type="text" name="sf_id_pracownik" id="sf_id_pracownik" placeholder="Wpisz ID..." pattern="{literal}^[\d]{1,4}${/literal}" required>
                        <input type="submit" id="submit" value="Szukaj" /> </li>
                </ul>
            </form>
        {/if}
        <div id="table">
            {include file="PracownikListTable.tpl"}
        </div>
        <ul class="actions special">
            {if count($conf->roles)>0 && isset($conf->roles['pracownik'])} 
                {if $conf->roles['pracownik']}
                    <li><a href="{url action="controlPanel"}" class="button alt">Powrót </a></li>
                    {/if}
                {/if}
                {if count($conf->roles)>0 && isset($conf->roles['admin'])} 
                    {if $conf->roles['admin']}
                    <li><a href="{url action="adminPanel"}" class="button alt">Powrót </a></li>
                    {/if}
                {/if}
        </ul>
    </section>
